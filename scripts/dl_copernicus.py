...
# This script from https://dataspace.copernicus.eu/ lets you download their
# 90m satellite maps (~250GB).
...
parser.add_argument(
    "-u", "--username", type=str, help="Username for authentication (required)"
)
parser.add_argument(
    "-p", "--password", type=str, help="Password for authentication (required)"
)
parser.add_argument(
    "eo_product_name",
    type=str,
    help="Name of the Earth Observation product to be downloaded (required)",
)
...
def get_access_token(config, username, password):
    ...
    response = requests.post(
        config["auth_server_url"], data=auth_data, verify=True, allow_redirects=False
    )
...
def get_eo_product_details(config, headers, eo_product_name):
    ...
    if response.status_code == 200:
        ...
        print(
            f"Failed to retrieve EO product details. Status code: {response.status_code}"
        )
        exit(1)
...
def get_temporary_s3_credentials(headers):
    ...
    if credentials_response.status_code == 200:
        ...
        print(
            f"Failed to create temporary S3 credentials. Status code: {credentials_response.status_code}"
        )
...
def download_file_s3(s3, bucket_name, s3_key, local_path, failed_downloads):
    ...
    try:
        ...
        with tqdm(
            ...
            s3.download_file(
                bucket_name, s3_key, local_path, Callback=progress_callback
            )
...
def traverse_and_download_s3(
    s3_resource, bucket_name, base_s3_path, local_path, failed_downloads
):
    """
...
def main():
    ...
    eo_product_id, s3_path = get_eo_product_details(
        config, headers, args.eo_product_name
    )
    ...
    traverse_and_download_s3(
        s3_resource, bucket_name, base_s3_path, top_level_folder, failed_downloads
    )
